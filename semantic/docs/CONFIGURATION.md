...
Numerical distribution/rate statistics apply to decoded values and command values. Boolean and known double-point states, Causes of Transmission, quality masks, synchronization codes, configuration revisions and sequence counters instead have observation/change counts and first/last code references. The analyzer does not average category numbers or interpret a counter mean as a physical measurement; modulo validity belongs to the protocol module.
